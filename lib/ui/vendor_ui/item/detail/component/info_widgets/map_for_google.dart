...
class _MapForGoogleState extends State<MapForGoogle> {
  ...
  Widget build(BuildContext context) {
    ...
    final googlemap.CameraPosition kGooglePlex = googlemap.CameraPosition(
      target: googlemap.LatLng(double.tryParse(product.lat ?? '0.0') ?? 0.0, double.tryParse(product.lng ?? '0.0') ?? 0.0),
      zoom: zoom,
    );
    return googlemap.GoogleMap(
        ...
        circles: <googlemap.Circle>{}..add(googlemap.Circle(
            ...
            center: googlemap.LatLng(double.tryParse(product.lat ?? '0.0') ?? 0.0, double.tryParse(product.lng ?? '0.0') ?? 0.0),
            radius: 50,
